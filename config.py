import os

DB_DETAILS = {
    'dev': {
        'SOURCE_DB': {
            'DB_TYPE': 'mysql',
            # Change the default from 'mysql_container' to '127.0.0.1' or 'localhost'
            'DB_HOST': os.environ.get('SOURCE_DB_HOST', '127.0.0.1'),
            'DB_NAME': os.environ.get('SOURCE_DB_NAME', 'retail_db'),
            'DB_USER': os.environ.get('SOURCE_DB_USER', 'root'),
            'DB_PASS': os.environ.get('SOURCE_DB_PASS', 'Password@123456')
        },
        'TARGET_DB': {
            'DB_TYPE': 'postgres',
            # Change the default from 'postgres_container' to '127.0.0.1' or 'localhost'
            'DB_HOST': os.environ.get('TARGET_DB_HOST', '127.0.0.1'),
            'DB_NAME': os.environ.get('TARGET_DB_NAME', 'retail_db'),
            'DB_USER': os.environ.get('TARGET_DB_USER', 'postgres'),
            'DB_PASS': os.environ.get('TARGET_DB_PASS', 'Post@123456')
        }
    }
}